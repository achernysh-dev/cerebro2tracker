...
Пакет pycerebro использует сторонний пакет psycopg2 (http://initd.org/psycopg/) 
...
from pycerebro import dbtypes, database, cargador, cclib # в этом модуле описаны различные константы, такие как поля данных, флаги и т.п.


# Переменные, которые вам возможно придется изменить, чтобы преспособить сктипт для вашей сети
mirada_path = 'C:/Program Files (x86)/Cerebro/mirada.exe' # '//ss/front/cerebro/mirada.exe' # Путь, откуда запускать мираду для генерации эскизов.
#У вас этот параметр скорее всего будет иным.

cargador_host = 'storage2.cerebrohq.com' # 'ss' # Cетевой адрес машины, где работает севрис каргадор.
